...
class TestStampAssets:
    def test_bundled_svgs_exist(self):
    ...
    def test_stamp_bounds_bottom_left_anchor(self):
        width, height = stamp_pixel_size("sample-star.svg", 100)
        left, top, right, bottom = stamp_bounds("sample-star.svg", 20, 200, 100)
        assert left == 20
        assert bottom == 200
...
class TestStampDocument:
    def test_add_and_hit_test(self, tmp_path, monkeypatch):
        ...
        stamp = doc.add_stamp("sample-star.svg", 50, 180, 90, 40, "soft_light", None)
        assert stamp.name == "Stamp 1"
        assert doc.find_stamp_at_point(50, 180) == 0
        assert doc.find_stamp_at_point(50, 180 - 90) >= 0
    ...
    def test_move_selected_stamp(self, tmp_path):
        ...
        doc = Document(image_path, Settings())
        doc.add_stamp("sample-star.svg", 10, 100, 50, 30, "normal", None)
        doc.select_stamp(0)
    ...
    def test_preview_renders_stamp_at_scaled_size(self, tmp_path):
        ...
        doc = Document(image_path, Settings())
        doc.add_stamp("sample-badge.svg", 200, 400, 438, 100, "normal", None)
...
class TestStampHitTest:
    def test_stamp_hit_test_helper(self):
        ...
        assert stamp_hit_test(stamp, 30, 120)
        assert not stamp_hit_test(stamp, 0, 0)
